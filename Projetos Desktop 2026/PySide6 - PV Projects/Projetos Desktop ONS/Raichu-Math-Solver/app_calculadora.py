...
class FunctionModel(QObject):
    ...
    def set_function(self, expression_str: str):
        ...
        try:
            ...
            self.current_function = sp.sympify(
                expression_str,
                locals={
                    "x": self.x,
                    "y": self.y,
    ...
    def evaluate(self, **kwargs):
        ...
        try:
            subs_list = [
                (getattr(self, k), v) for k, v in kwargs.items() if hasattr(self, k)
            ]
...
class CalculusModel(QObject):
    ...
    def solve_ode(self, ode_str: str, func_name: str = "y"):
        ...
        try:
            ...
            ode_expr = sympify(
                ode_str, locals={"y": y, "t": t, "Derivative": Derivative}
            )
    ...
    def execute(self, code: str):
        ...
        try:
            ...
            result = subprocess.run(
                [self.julia_path, temp_file], capture_output=True, text=True, timeout=30
            )
            ...
            self.output_signal.emit(
                "", "Julia não encontrado. Instale Julia e adicione ao PATH."
            )
...
"""
    ...
    def run_script(self):
        ...
        if code.strip():
            ...
            self.output_text.append(
                f"Executando em {datetime.now().strftime('%H:%M:%S')}..."
            )
    ...
    def handle_console_command(self, command: str, console: ConsoleWidget):
        ...
        if cmd_lower == "help":
            ...
            console.append_output(
                "Funções: sin, cos, tan, exp, log, sqrt, pi", color="#4fc1ff"
            )
            console.append_output(
                "Cálculo: diff, integrate, Matrix, solve, dsolve", color="#4fc1ff"
            )
            ...
            console.append_output(
                "Use a aba 'Configuração' para salvar funções", color="#ce9178"
            )
...
class CalculatorMainWindow(QMainWindow):
    ...
    def plot_current_function(self):
        ...
        x_range = (
            self.config_widget.x_range_min.value(),
            self.config_widget.x_range_max.value(),
        )
        y_range = (
            self.config_widget.y_range_min.value(),
        ...
        success = self.plot_widget.plot_function(
            func, x_range, y_range, resolution, plot_type
        )
