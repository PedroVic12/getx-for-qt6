...
# 1. styles.py
cat <<EOF > $PROJECT_NAME/styles.py
# styles.py
COMMON_STYLES = """
...
# 2. CORE: base_view.py
cat <<EOF > $PROJECT_NAME/core/base_view.py
...
# 3. CORE: database.py
cat <<EOF > $PROJECT_NAME/core/database.py
import sqlite3
...
cat <<EOF > $PROJECT_NAME/runtime_imports.py
...
cat <<EOF > $PROJECT_NAME/views/components/text_button.py
...
cat <<EOF > $PROJECT_NAME/views/components/checklist_widget.py
...
cat <<EOF > $PROJECT_NAME/views/pages/help_view.py
...
cat <<EOF > $PROJECT_NAME/views/pages/settings_view.py
from PySide6.QtWidgets import QLabel
from core.base_view import StatelessView
